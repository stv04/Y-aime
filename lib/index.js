...
export function postearAlgo(nombre, uid){
	...
	.then(function(docRef) {
    	...
    	mensaje.textContent = 'Has posteado algo';
    ...
    if (doc.exists) {
		...
		.addEventListener('click', () => {
			...
			.then(function() {
    			...
    			mensaje.textContent = 'Has editado un post';
		    	mensaje.style.display = 'block';
		    	setInterval(() => {
		    		mensaje.style.display = 'none';
		    	}, 4000);
...
function eliminarPublicacion(docu) {
	...
	.then(function() {
    	...
    	mensaje.textContent = 'Has eliminado un post';
    	mensaje.style.display = 'block';
    	setInterval(() => {
    		mensaje.style.display = 'none';
    	}, 4000);
...
function cambiarPerfilDeUsuario(dato1, dato2, foto){
  ...
  // Update successful.
  		...
  		mensaje.textContent = 'HCambio de nobre registrado.';
    	mensaje.style.display = 'block';
    	setInterval(() => {
    		mensaje.style.display = 'none';
    	}, 4000);
...
function subirGustos() {
  ...
  // Update successful.
  		...
  		mensaje.textContent = 'Tus gustos han sido registrados';
    	mensaje.style.display = 'block';
    	setInterval(() => {
    		mensaje.style.display = 'none';
    	}, 4000);
...
function mensajeDeVerificacion() {
	...
	user.sendEmailVerification().then(function() {
	  ...
	  document.getElementById('seccion-emergente').innerHTML = `
		...
		dirigase a la bandeja de entrada de su Email para autenticar su cuenta, 
		luego vuelva y recargue la página.</h2>`
	}).catch(function(error) {
	  // An error happened.
	  console.log(error)
	});
